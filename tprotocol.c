...
#define PKTSIZE_OFFSET     ( PREAMB_SIZE )
#define SRCADDR_OFFSET     ( PKTSIZE_OFFSET + 1 )   // 5
#define DSTADDR_OFFSET     ( SRCADDR_OFFSET + 1 )   // 6
#define DATA_OFFSET        ( DSTADDR_OFFSET + 1 )   // 7
...
prResult_t   prGetByte( prListener_t* listener, uint8_t byte )
{
	...
	listener->pktFrame[ listener->pos ] = byte;
      ...
      case PRS_CHECKSUM:
        n = listener->pos - ( listener->pktDataSize + 7 );
		
        if ( n > 2 ) 
        {
            crc16 = prChecksum( listener->pktFrame, listener->pos  );
            if ( crc16 != *( uint16_t* )( &listener->pktFrame[ listener->pos - 2 ] ) ) 
            {
                return PR_FAIL;
            }
			else {
...
uint16_t prChecksum( uint8_t* buffer, uint8_t size )
{
    ...
    while ( size-- )
        crc = (crc << 8) ^ Crc16Table[(crc >> 8) ^ *buffer++];
